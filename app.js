    ...
    updateGeneSetStatus() {
        ...
        if (this.useCustomSelection) {
            ...
            this.showStatus('geneSetStatus', 'info',
                `Custom selection: ${n.toLocaleString()} gene sets selected. Click "Browse" to change`);
            return;
        }
...
`;
    ...
    _showRerunDialog(title, info, filtered, suggestedPerms) {
        ...
        document.getElementById('rerunPermHint').textContent =
            prevPerms <= 500
                ? `Previous run used ${prevPerms} permutations. Consider increasing to ≥1000 for accurate p-values.`
    ...
    renderGeneSetInfo(geneSetName) {
        ...
        el.innerHTML = `
            ...
            <div style="margin-top: 6px; display: flex; gap: 10px; flex-wrap: wrap;">
                ...
                ${this.canShowHeatmap && this.canShowHeatmap() ? `<a href="#" onclick="app.openGeneSetHeatmap('${this._escapeAttr(geneSetName)}'); return false;" style="font-size: 0.85em; color: var(--green-600); text-decoration: none; font-weight: 600;" title="${this.hasComparisonData() ? 'Heatmap of this gene set across the cell lines of group A and group B, with the filters that defined the groups' : 'The values of this gene set\'s genes in the loaded cell line as a bar chart, leading edge genes in green'}">&#9638; ${this.hasComparisonData() ? 'Heatmap by cell line' : 'Gene values in this cell line'}</a>` : ''}
            </div>
