...
## DepMap data
- `depmapService.js` reads one cell line row from `web_data/depmap_{crispr,expression}.bin` (int16 x1000, row-major) using `web_data/depmap_index.json`; built by `prepare_depmap_matrix.py` from files symlinked into the git-ignored `web_data/.cache/`. Expression keeps one profile per model (IsDefaultEntryForModel). The five quick example buttons map to DepMap IDs inside `loadExampleData`.
